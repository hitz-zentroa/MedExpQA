...
if __name__ == "__main__":
    fnames = sorted([fname for fname in os.listdir("data/rag/statpearls/statpearls_NBK430685") if fname.endswith("nxml")])
    if not os.path.exists("data/rag/statpearls/chunk"):
        os.makedirs("data/rag/statpearls/chunk")
    for fname in tqdm.tqdm(fnames):
        fpath = os.path.join("data/rag/statpearls/statpearls_NBK430685", fname)
        saved_text = extract(fpath)
        if len(saved_text) > 0:
            with open("data/rag/statpearls/chunk/{:s}".format(fname.replace(".nxml", ".jsonl")), 'w') as f:
                f.write('\n'.join(saved_text))